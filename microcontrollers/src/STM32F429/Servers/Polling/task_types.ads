...
package Task_Types is
   ...
   task type Poller_Server (Period       : access constant Ada.Real_Time.Time_Span;
                            Worker_Queue : access Task_Control.Job_Queue) with Priority => System.Max_Priority;
   task type ADC_Handler (Period : access constant Ada.Real_Time.Time_Span) with Priority => System.Max_Priority - 1;
   task type EXTI4_Handler (Worker_Queue : access Task_Control.Job_Queue) with Priority => System.Max_Priority - 2;
   task type Green_Light_Controller (Period       : access constant Ada.Real_Time.Time_Span;
                                     Worker_Queue : access Task_Control.Job_Queue) with Priority => System.Max_Priority - 3;
